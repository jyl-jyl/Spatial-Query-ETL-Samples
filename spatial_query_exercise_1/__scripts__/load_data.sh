...
POSTGRES_NAME=${POSTGRES_NAME:-musa_509_assignment02}
POSTGRES_PORT=${POSTGRES_PORT:-5432}
POSTGRES_USER=${POSTGRES_USER:-ljy}
POSTGRES_PASS=${POSTGRES_PASS:-postgresql_jyl}
...
# SCRIPTDIR=$(realpath "$dirname")
# DATADIR=$(realpath "$dirname"/../__data__)

SCRIPTDIR=${SCRIPTDIR:-__scripts__/load_data.sh}
DATADIR=${DATADIR:-__data__}

mkdir -p ${DATADIR}

# pathname="$(realpath "$dirname")"
...
PGPASSWORD=postgres psql \
  ...
  -U ${POSTGRES_USER} \
  -d ${POSTGRES_NAME} \
  -f "__scripts__/create_tables.sql"
